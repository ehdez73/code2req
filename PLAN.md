...
- [x] Verify: `mvn test` (366 total — 24 original + 5 @Query/@NamedQuery tests + 5 raw JDBC tests + 1 npjt scope test + 1 Spring Data JDBC @Query import test)
...
### Phase 14 — Snapshot & Restore (Epic E005)

Cross-cutting operational capability for point-in-time save and restore of all local state (SQLite DB + JSON index). Not tied to any specific analysis phase — works after scan, after enrichment, or at any point.
...
- 2026-06-18 — **Phase 9.1 F011 extended** (Custom SQL/HQL + Raw JDBC detection + scope broadening): Added `DbAccessType.NATIVE_SQL`/`JPQL_HQL`; `NamedQueryDetector` (`@NamedQuery`/`@NamedNativeQuery`), `RawJdbcDetector` (`Connection`/`Statement`); updated `SpringDataJpaDetector` (no longer skips `@Query`, import-aware Spring Data JDBC routing), `EntityManagerDetector` and `HibernateSessionDetector` (route query methods to `NATIVE_SQL`/`JPQL_HQL`); broadened `JdbcTemplateDetector` scope to match `npjt` (`NamedParameterJdbcTemplate`); 12 new tests — 366 total, all pass ✓

...
public CompletableFuture<ExecutionFinding> enrich(Task task,
        ...
        if (enrichmentConfig.resolvedExecutionMode() == ExecutionMode.SYNC) {
            return executeSync(task, sourceContent, testContent, structuralContextJson, dryRun);
        }
        return executeAsync(task, sourceContent, testContent, structuralContextJson, dryRun);
    }

    private CompletableFuture<ExecutionFinding> executeSync(Task task,
                                                              String sourceContent, String testContent,
                                                              String structuralContextJson, boolean dryRun) {
        try {
            ExecutionFinding finding = doEnrich(task, sourceContent, testContent, structuralContextJson, dryRun);
    ...
    private CompletableFuture<ExecutionFinding> executeAsync(Task task,
                                                               String sourceContent, String testContent,
                                                               String structuralContextJson, boolean dryRun) {
        var future = new CompletableFuture<ExecutionFinding>();
        Runnable work = () -> {
            try {
                future.complete(doEnrich(task, sourceContent, testContent, structuralContextJson, dryRun));
            } catch (Exception e) {
                log.error("Failed to enrich task {}: {}", task.taskId(), e.getMessage());
                taskStore.updateStatus(task.taskId(), TaskStatus.ENRICH_FAILED);
    ...
    private ExecutionFinding doEnrich(Task task,
                                       String sourceContent, String testContent,

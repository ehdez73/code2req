...
> As a **Developer**, I want **the system to aggregate all Phase 1 and Phase 2 data into a single queryable domain model**, so that **the Embabel agent can discover entry points, traverse the call graph, and detect orphaned methods holistically**.
...
- [ ] `getEntryPoints()` returns all entry points (HTTP, SCHEDULED, KAFKA, RABBITMQ, ACTIVEMQ, EVENT_LISTENER) with their finding JSON
- [ ] `getCallersOf(target)` and `getCalleesOf(source)` provide directed graph traversal
- [ ] `getComponentsByType(type)` filters components by annotation type
- [ ] `getAllKnownMethods()` returns all methods defined across all components for orphaned method detection
